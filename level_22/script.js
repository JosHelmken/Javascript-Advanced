...
const heroesMarvel = superheroes.filter(
 ...
 const heroWeights = superheroes.map(n => n.weight);

 const totalWeight = (heroWeights = []) => {
   let sum = 0;
   for(let i = 0; i < heroWeights.length; i++){
     const el = heroWeights[i]; if(+el) {
       sum += +el;
     }
   }
   return sum;
 ...
 console.log(totalWeight(heroWeights))
